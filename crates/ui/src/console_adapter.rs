...
use halo_core::{
    ConsoleCommand, ConsoleEvent, ConsoleHandle, CueList, MidiOverride, PlaybackState, RhythmState,
    Show, TimeCode,
};
...
impl ConsoleAdapter {
        ...
        self.runtime.spawn(async move {
            ...
            while let Ok(event) = rx.try_recv() {
                ...
                match event {
                    ...
                    ConsoleEvent::PlaybackStateChanged {
                        state: playback_state,
                    } => {
                        state.playback_state = playback_state;
                    }
                    ConsoleEvent::RhythmStateUpdated {
                        state: rhythm_state,
                    } => {
                    ...
                    ConsoleEvent::CurrentPlaybackState {
                        state: playback_state,
                    } => {
                        state.playback_state = playback_state;
                    }
                    ConsoleEvent::CurrentRhythmState {
                        state: rhythm_state,
                    } => {
                        state.rhythm_state = rhythm_state;
                    }
    ...
    pub fn play_cue(&self, list_index: usize, cue_index: usize) -> Result<(), String> {
        self.send_command(ConsoleCommand::PlayCue {
            list_index,
            cue_index,
        })
    }
    ...
    pub fn add_midi_override(&self, note: u8, override_config: MidiOverride) -> Result<(), String> {
        self.send_command(ConsoleCommand::AddMidiOverride {
            note,
            override_config,
        })
    }
