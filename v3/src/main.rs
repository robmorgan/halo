...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cues = vec![
        Cue {
            name: "Color Fade".to_string(),
            duration: 10.0,
            effect_mappings: vec![EffectMapping {
                effect: Effect {
                    name: "Constrained Sine".to_string(),
                    apply: sine_wave_effect,
                    min: 16384, // Minimum at 25%
                    max: 49151, // Maximum at 75%
                },
                fixture_names: vec!["Moving Head 1".to_string(), "Moving Head 2".to_string()],
                channel_types: vec![ChannelType::Red, ChannelType::Green, ChannelType::Blue],
                distribution: EffectDistribution::Wave(std::f64::consts::PI),
            }],
            static_values: vec![
                StaticValue {
        ...
        Cue {
            name: "Pan Tilt Movement".to_string(),
            duration: 15.0,
            effect_mappings: vec![EffectMapping {
                effect: effects[1].clone(),
                fixture_names: vec!["Moving Head 1".to_string(), "Moving Head 2".to_string()],
                channel_types: vec![ChannelType::Pan, ChannelType::Tilt],
...
fn sine_wave_effect(_current: u16, time: f64, _cue_time: f64, _delta: f64) -> f64 {
    (time.sin() * 0.5 + 0.5)
}
